    ...
    func setModel(aMessage: SBDUserMessage, channel: SBDBaseChannel?) {
        ...
        if self.message.channelType == CHANNEL_TYPE_GROUP {
            if self.message.messageId == 0 {
                self.imgMessageStatus.image = UIImage(named: "icMsgsent", in: Bundle.bundleForXib(OutgoingLocationMessageTableViewCell.self), compatibleWith: nil)
            }
            else {
                if let channelOfMessage = channel as? SBDGroupChannel? {
                    ...
                    if unreadMessageCount == 0 {
                        ...
                        self.imgMessageStatus.image = UIImage(named: "icMsgread", in: Bundle.bundleForXib(OutgoingLocationMessageTableViewCell.self), compatibleWith: nil)
                    }
                    else {
                        self.imgMessageStatus.image = UIImage(named: "icMsgdelivered", in: Bundle.bundleForXib(OutgoingLocationMessageTableViewCell.self), compatibleWith: nil)
                    }
                }
            }
        }
